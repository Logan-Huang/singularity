...
pub enum ErrorCorrectionMode {
    ...
    Disabled = ffi::msdfgen_ErrorCorrectionConfig_Mode_DISABLED as u32,

    /// Corrects all discontinuities of the distance field regardless if edges are adversely affected
    Indiscriminate = ffi::msdfgen_ErrorCorrectionConfig_Mode_INDISCRIMINATE as u32,

    /// Corrects artifacts at edges and other discontinuous distances only if it does not affect edges or corners
    EdgePriority = ffi::msdfgen_ErrorCorrectionConfig_Mode_EDGE_PRIORITY as u32,

    /// Only corrects artifacts at edges
    EdgeOnly = ffi::msdfgen_ErrorCorrectionConfig_Mode_EDGE_ONLY as u32,
}
...
pub enum DistanceCheckMode {
    ...
    DoNotCheck = ffi::msdfgen_ErrorCorrectionConfig_DistanceCheckMode_DO_NOT_CHECK_DISTANCE as u32,

    /// Only computes exact shape distance at edges. Provides a good balance between speed and precision
    CheckAtEdge = ffi::msdfgen_ErrorCorrectionConfig_DistanceCheckMode_CHECK_DISTANCE_AT_EDGE as u32,

    /// Computes and compares the exact shape distance for each suspected artifact
    AlwaysCheck = ffi::msdfgen_ErrorCorrectionConfig_DistanceCheckMode_ALWAYS_CHECK_DISTANCE as u32,
}
