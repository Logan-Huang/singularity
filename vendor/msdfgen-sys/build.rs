...
fn generate_bindings<P: AsRef<Path>>(
    ...
    let bindings = bindgen::Builder::default()
        ...
        .clang_arg("-xc++")
        // PATCHED (was c++11): MSVC's STL headers need C++14+ to parse
        .clang_arg("-std=c++17")
    ...
    bindings.write_to_file(out_file.as_ref()).expect("Written bindings.");

    // PATCHED: on MSVC targets, bindgen mangles destructors as the "vbase
    // destructor" (??_D…QEAAXXZ), a symbol compilers only synthesize inline
    // for classes without virtual bases — so it never exists in the compiled
    // library. Retarget those bindings at the real destructor (??1…QEAA@XZ).
    let generated =
        std::fs::read_to_string(out_file.as_ref()).expect("Read generated bindings.");
    if generated.contains("??_D") {
        let patched = generated
            .lines()
            .map(|line| {
                if line.contains("??_D") {
                    line.replace("??_D", "??1").replace("QEAAXXZ", "QEAA@XZ")
                } else {
                    line.to_string()
                }
            })
            .collect::<Vec<_>>()
            .join("\n");
        std::fs::write(out_file.as_ref(), patched).expect("Written patched bindings.");
    }
}
